...
if($action=='add'){
	...
	$events=mysql_real_escape_string(strip_tags($events),$conn); //过滤HTML标签，并转义特殊字符

	$isallday = $_POST['isallday'];//是否是全天事件
	...
	$query = mysql_query("insert into `calendar` (`title`,`starttime`,`endtime`,`allday`,`color`,`uid`) values ('$events','$starttime','$endtime','$isallday','$color'," . $_SESSION['memo_id'] . ")");
